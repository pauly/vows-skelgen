...
skelgen.assertsFrom = function ( comment ) {
  var regex = new RegExp( '^\\s+\\*\\s+@assert\\s+\\((.+)\\)\\s+(={2,3})\\s+(.+?)$', 'gm' ); // backup
  var match;
  var asserts = [ ];
...
skelgen.goodInputTests = function ( method, params ) {
  ...
  if ( fixtures && fixtures.length && args.output ) {
    ...
    for ( i = 0; i < fixtures.length; i ++ ) {
      ...
      test += ( i ? ',\n' : '' ) +
        '    \'handles fixture ' + path.basename( relativeFixture ) + '\': function ( ) {\n' +
...
skelgen.goodInputTestsAsync = function ( method, params, fileToTest ) {
  ...
  if ( asserts && asserts.length ) {
    ...
    for ( i = 0; i < asserts.length; i ++ ) {
      ...
      test += '      test' + i + ': {\n' +
        '        topic: function ( ) {\n' +
        '          ' + method + '( ' + ( asserts[i][0] || 'undefined' ) + ', this.callback );\n' +
        '        },\n' +
        '        callback: function( err, result ) {\n' +
...
skelgen.generate = function ( ) {
  ...
  console.warn( 'warning: deprecated, checking out the mocha2 branch instead' );
  if ( ! args.file ) {
  ...
  /* if ( args.hx ) {
    ...
    [ 'config', 'Token' ].forEach( function ( module ) {
      var cmd = 'find ' + process.cwd( ) + ' -name ' + module + ' | head -1';
