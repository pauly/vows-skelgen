...
skelgen.asyncTests = function ( method, args, file ) {
  ...
  test += '    describe( \'handles malformed input\', function ( ) {\n';
...
skelgen.goodInputTests = function ( method, params, file ) {
  ...
  if ( asserts && asserts.length ) {
    for ( var i = 0; i < asserts.length; i ++ ) {
      test += '      var data = ' + asserts[i][0] + ';\n';
      test += '      var expected = ' + asserts[i][2] + ';\n';
      test += '      assert.deepEqual( ' + method + '( data ), expected );\n';
    }
  }
  else {
...
skelgen.goodInputTestsAsync = function ( method, params, file ) {
  var test = '    describe( \'handles good input\', function ( ) {\n';
  var asserts = skelgen.assertsFrom( skelgen.commentBefore( skelgen.uninstantiate( method ), file ));
  if ( asserts && asserts.length ) {
    for ( var i = 0; i < asserts.length; i ++ ) {
    ...
    test += '      // it( \'handles ' + skelgen.sanitise( params ) + '\', function ( done ) {\n' +
      '        // ' + method + '( ' + params + ', function( err, result ) {\n' +
      '        // assert.deepEqual( err, null );\n' +
      '        // done( );\n' +
      '      // } );\n';
